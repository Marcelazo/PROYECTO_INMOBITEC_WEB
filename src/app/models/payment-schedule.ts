...
export class PaymentSchedule {
  ...
  get invoiceName(): string {
    let property = '';
    let name = '';
    if (this.preSale?.property) {
      const { manzana, lote } = this.preSale.property;
      property = `MZ${manzana ?? ''}_LT${lote ?? ''}`;
    }

    if (this.ticket) {
      const { serieDoc, nroDoc } = this.ticket;
      name = `${serieDoc ?? ''}-${nroDoc ?? ''}`;
    }

    return `BOLETA_${property}_${name}.pdf`;
  }
}
